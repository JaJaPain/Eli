 ...
 * Role: Environmental Logic Controller (Massive Pile Edition)
 */
...
const MAX_LINGERING = 3000; // Increased capacity
...
class Sand {
    constructor(x, y) {
        ...
        this.size = random(1.5, 2.5); // Slightly larger grains
...
class LingeringGrain {
    ...
    update() {
        this.alpha -= 0.8; // MUCH SLOWER FADE (lasts ~4-5 seconds)
    }
    display(gfx) {
        gfx.noStroke();
...
function draw() {
    ...
    // BOTTOM SAND - MASSIVE INCREASE IN PILE HEIGHT (65 vs 25)
    let currentPileHeight = progress * 65; 
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
        ...
        if (particles[i].dead) {
            if (collided && random() < 0.6 && lingeringGrains.length < MAX_LINGERING) {
                // Stack slightly offset to build "side volume"
    ...
    // Solid Pile (Tuned to the new height)
    ...
    sandGfx.fill(0, 45); sandGfx.arc(0, 62, 110, currentPileHeight * 2, PI, TWO_PI);
    ...
    pop();
    push(); resetMatrix(); image(sandImg, width/2, height/2); image(topImg, width/2, height/2); pop();
